 ...
 *  LANGAGE : TypeScript + JSX (React) — Framework Next.js
 *  RÔLE    : La PAGE PRINCIPALE de l'application (le tableau de bord gérance).
 *            C'est le "chef d'orchestre" : il décide quel écran afficher
 *            selon l'onglet choisi dans la barre latérale (sidebar).
 *
 *  Structure :
 *    1. ÉTATS (useState)     → mémorisent les produits, commandes, clients...
 *    2. loadAllData()        → va chercher les données dans les API (fetch)
 *    3. Gestionnaires        → fonctions appelées quand on ajoute/supprime
 *    4. RENDU (return JSX)   → l'interface visuelle :
 *       - Barre latérale de navigation
 *       - En-tête avec recherche et boutons rapides
 *       - Zone centrale qui change selon l'onglet actif
 *         (overview, pos, products, orders, customers, suppliers...)
 *
 *  Si activeTab === "storefront", on affiche la VITRINE CLIENT (la boutique
 *  en ligne visible par vos clients) au lieu du tableau de bord.
 * ==========================================================================*/

"use client"; // Indique à Next.js que ce composant utilise le navigateur (interactions)
...
export default function YarwayeShopPage() {
  ...
  // Handlers for optimistic and reactive updates
  ...
  const navItems = [
    ...
    { id: "pos", label: "Caisse & POS", icon: ShoppingCart, badge: "Vente" },
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col antialiased">
      ...
      {/* Top Banner with prominent hotline */}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span>Ouvrir la Boutique Client (Storefront)</span>
            </button>
          </div>
        </div>
      </div>
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="truncate">
                  <p className="text-xs font-bold text-white truncate">{user?.name || "Yarwaye Staff"}</p>
                  <p className="text-[10px] text-slate-400 capitalize">{user?.role || "Propriétaire"}</p>
                </div>
              </div>

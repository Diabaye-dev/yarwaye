...
/* ============================================================================
 *  FICHIER : src/components/SettingsView.tsx
 *  LANGAGE : TypeScript + JSX (React)
 *  RÔLE    : L'écran "Paramètres Boutique" — configuration générale.
 *
 *  L'utilisateur peut modifier :
 *    - L'identité : nom (AGRO SERVICE YARWAYE), devise (FCFA), adresse
 *    - Les contacts : téléphone 767866536, WhatsApp, comptes Wave/Orange Money
 *    - Le ticket de caisse : en-tête et message de pied de page
 *
 *  handleSave()   → PUT  /api/settings  (enregistre les modifications)
 *  handleResetDemoData() → POST /api/seed (réinitialise les données démo)
 * ==========================================================================*/
...
export function SettingsView({ settings, onRefresh }: SettingsViewProps) {
  const [formData, setFormData] = useState({
    ...
    email: settings?.email || "contact@agroserviceyarwaye.sn",
    address: settings?.address || "Malika QRT Malika / MER - BP 17000 - Keur Massar, Dakar, Sénégal",
    currency: settings?.currency || "FCFA",
    taxRate: settings?.taxRate || 0,
    receiptHeader: settings?.receiptHeader || "AGRO SERVICE YARWAYE - AGRICULTURE & COMMERCE GÉNÉRAL",
    receiptFooter: settings?.receiptFooter || "Service Client & Wave/OM: 767866536. Merci de votre fidélité !",
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Téléphone Principal / Hotline Boutique
              </label>

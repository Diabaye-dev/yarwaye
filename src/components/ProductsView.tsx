"use client";
/**
 * VUE "Inventaire & articles" — src/components/ProductsView.tsx
 * 
 * Liste complète du catalogue avec recherche, filtres catégorie / stock bas /
 * rupture, marges par article, export CSV et actions (ajuster le stock,
 * modifier, supprimer).
 */
...
export function ProductsView({ products, settings, onRefresh, onProductDeleted }: ProductsViewProps) {
  ...
  // Catégories présentées dans le sélecteur.
  const categories = useMemo(() => {
    const set = new Set(products.map((p) => p.category));
    return ["all", ...Array.from(set)];
  }, [products]);

  // Application cumulée des trois filtres : texte, catégorie, niveau de stock.
  ...
  // Suppression avec confirmation, puis retrait immédiat du state (optimiste).
  ...
  // Génère un fichier CSV à la volée (data URI) pour l’inventaire filtré.
  const exportCSV = () => {
    ...
    link.setAttribute("download", `Inventaire_AGRO_SERVICE_YARWAYE_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const lowStockTotal = products.filter((p) => p.stock <= p.minStockAlert).length; // nombre d’alertes

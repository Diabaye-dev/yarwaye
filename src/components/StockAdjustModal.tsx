...
/* ============================================================================
 *  FICHIER : src/components/StockAdjustModal.tsx
 *  LANGAGE : TypeScript + JSX (React)
 *  RÔLE    : La modale d'AJUSTEMENT DE STOCK (+ / −) pour un produit.
 *            Exemple : réception d'un camion de maïs, casse, inventaire...
 *
 *  L'utilisateur choisit :
 *    - le sens : ENTRÉE (+ approvisionnement) ou SORTIE (− déstockage)
 *    - la quantité et le motif du mouvement
 *
 *  handleSubmit() appelle POST /api/products/:id/stock qui :
 *    1. Met à jour la quantité en stock du produit
 *    2. Écrit une ligne dans le journal des mouvements (stock_movements)
 *       avec l'ancien stock, le nouveau stock, le motif et l'opérateur
 * ==========================================================================*/
    ...
    try {
      const res = await fetch(`/api/products/${product.id}/stock`, {
        ...
        body: JSON.stringify({
          ...
          type: mode === "in" ? "in" : "out",
          reason: reason,
          performedBy: user?.name || "Yarwaye Staff",
        }),
      });
